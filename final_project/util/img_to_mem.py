...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        image_out.save('num_preview.png')
        print('Output image preview saved at num_preview.png')
        palette = image_out.getpalette()
        rgb_tuples = [tuple(palette[i:i+3]) for i in range(0, 3*num_colors_out, 3)]

        # Save pallete
        with open(f'word_palette.mem', 'w') as f:
            f.write( '\n'.join( [f'{r:02x}{g:02x}{b:02x}' for r, g, b in rgb_tuples] ) )

        print('Output image pallete saved at word_palette.mem')

        # Save the image itself
        with open(f'num_image.mem', 'w') as f:
            for y in range(h):
                for x in range(w):
                    f.write(f'{image_out.getpixel((x,y)):02x}\n')

        print('Output image saved at num_image.mem')
